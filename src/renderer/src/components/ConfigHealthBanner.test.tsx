...
describe("ConfigHealthBanner", () => {
  ...
  it("ignores the obsolete missing gateway-key warning from a stale report", async () => {
    const stale = report([
      { code: "EMPTY_API_SERVER_KEY", severity: "warning" },
    ]);
    const api = {
      getConfigHealth: vi.fn().mockResolvedValue(stale),
    };
    Object.defineProperty(window, "hermesAPI", {
      configurable: true,
      value: api,
    ...
    expect(screen.queryByTestId("config-health-banner")).toBeNull();
    expect(Object.keys(api)).toEqual(["getConfigHealth"]);
  });
});
