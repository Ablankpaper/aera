...
export function buildGatewayStartupDiagnosticScript(profile?: string): string {
  ...
  return [
    "import faulthandler,json,os,runpy,sys,time",
    `TARGETS=frozenset(${JSON.stringify(TARGET_FUNCTIONS)})`,
    `TARGET_FILES=tuple(${JSON.stringify(TARGET_FILES)})`,
    "TARGET_FILE_SUFFIXES=tuple(part.lower() for part in TARGET_FILES)",
    ...
    // Keep this profiler on the launch thread only.  Applying it to every
    // future thread made the diagnostic execute inside logging/argparse
    // workers; the packaged Windows trace then crashed in its own filter.
    "def _profile(frame,event,arg):\n  if event not in ('call','return'): return _profile\n  try:\n    code=frame.f_code\n    short=code.co_name\n    qualified=getattr(code,'co_qualname',short)\n    if short not in TARGETS and qualified not in TARGETS: return _profile\n    filename=code.co_filename\n    if not filename.replace('\\\\','/').lower().endswith(TARGET_FILE_SUFFIXES): return _profile\n    key=(event,filename,qualified,frame.f_lineno)\n    if key in SEEN or len(SEEN) >= MAX_EVENTS: return _profile\n    SEEN.add(key)\n    _write('function-'+('enter' if event == 'call' else 'return'),function=qualified,file=filename.replace('\\\\','/').rsplit('/',1)[-1],line=frame.f_lineno,thread='main')\n  except Exception: pass\n  return _profile",
