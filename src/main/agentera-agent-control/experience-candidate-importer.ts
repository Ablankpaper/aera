...
export class ExperienceCandidateImporterError extends Error {
  readonly code: ExperienceCandidateImporterErrorCode;

  constructor(code: ExperienceCandidateImporterErrorCode) {
    super(`ExperienceCandidate import failed: ${code}.`);
    this.name = "ExperienceCandidateImporterError";
    this.code = code;
  }
}

export interface ExperienceCandidateImportClient {
...
export interface ExperienceCandidateImportDraftStore {
...
export interface ExperienceCandidateImportReceiptStore {
...
export interface ExperienceCandidateImportVersionCache {
  cacheVerifiedVersion(version: AgentVersion): AgentVersion;
  getVerifiedVersion(versionId: string): AgentVersion;
}

export interface ExperienceCandidateImporterOptions {
  database: AgenteraControlPlaneDatabase;
  client: ExperienceCandidateImportClient;
  candidates: ExperienceCandidateImportReceiptStore;
  drafts: ExperienceCandidateImportDraftStore;
  cache: ExperienceCandidateImportVersionCache;
...
function importerError(code: ExperienceCandidateImporterErrorCode): never {
  throw new ExperienceCandidateImporterError(code);
}
...
function mapClientError(error: AgenteraAgentControlClientError): never {
  switch (error.code) {
    case "workspace_forbidden":
      return importerError("workspace_forbidden");
    case "workspace_archived":
      return importerError("workspace_archived");
    case "workspace_owner_unavailable":
      return importerError("workspace_owner_unavailable");
    case "not_found":
...
function exactReceipt(
  receipt: LocalExperienceCandidateImport,
...
export class ExperienceCandidateImporter {
  private readonly options: ExperienceCandidateImporterOptions;
  private readonly owner: AgenteraRuntimeOwner;
  private readonly randomUUID: () => string;
  private readonly prepared = new Map<string, PreparedImport>();

  constructor(options: ExperienceCandidateImporterOptions) {
  ...
  async prepare(
    ...
    let candidate: CloudExperienceCandidateDetail;
    try {
      candidate = await this.options.client.getExperienceCandidate(
        workspaceId,
        candidateId,
      );
    } catch (error) {
    ...
    try {
      definition = await this.options.client.getWorkspaceDefinition(
        workspaceId,
    ...
    try {
      downloaded = await this.options.client.getVersion(
        definition.latest_version_id,
      );
    } catch (error) {
      if (error instanceof AgenteraAgentControlClientError) {
        return mapClientError(error);
  ...
  async confirm(
    ...
    if (
      input === null ||
      typeof input !== "object" ||
      input.confirmation !== "apply-approved-skill-to-latest"
    ) {
      return importerError("invalid_request");
    }
    const workspaceId = uuid(workspaceIdInput);
    ...
    const existing = this.options.candidates.findImport(prepared.candidateId);
    if (existing !== null) {
    ...
    try {
      current = await this.options.client.getWorkspaceDefinition(
        workspaceId,
    ...
    try {
      ...
      const raced = this.options.candidates.findImport(prepared.candidateId);
      if (raced !== null) {
      ...
      if (error instanceof ExperienceCandidateImporterError) throw error;
