...
export function buildWindowsUpdateHelperScript(options?: {
  ...
  return [
    ...
    "  return @($seen)",
    "}",
    "function Get-TargetExecutableProcessIds {",
    "  $expected = [System.IO.Path]::GetFullPath($TargetExecutable)",
    "  $matches = @(Get-CimInstance Win32_Process -ErrorAction Stop | Where-Object {",
    "    $_.ExecutablePath -and ([System.IO.Path]::GetFullPath([string]$_.ExecutablePath) -eq $expected)",
    "  })",
    ...
    "      }",
    "      if (-not $running -and @(Get-TargetExecutableProcessIds).Count -gt 0) { $running = $true }",
