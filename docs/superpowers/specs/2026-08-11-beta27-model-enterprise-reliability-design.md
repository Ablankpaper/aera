...
An independent Desktop database at Electron `userData/model-configuration/model-configuration.db` stores `desktop_model_configuration_operations`; it never follows the active Hermes Profile or its `state.db`. Its directory is mode `0700` and its database is mode `0600` where POSIX modes apply. The journal contains only operation ID, owner/Profile handles, stage, canonical non-secret old/new route identities, symbolic file roles, before/after digests, existence flags, and timestamps. It never contains key values, absolute paths, or raw file bodies.

Before any product file changes, Main writes exact sibling backup files for the five allowlisted configuration paths with exclusive creation and restrictive permissions. Backup names are derived only from the operation UUID, remain beside the protected source document, and are removed after a verified commit or rollback. If the process exits mid-operation, startup recovery derives the same allowlisted paths from the validated Profile handle, verifies the journal and backups, then either recognizes a fully committed new active route or restores every prior byte/existence state and old active route. A missing/tampered backup or digest mismatch becomes `model_configuration_recovery_required`; recovery never guesses, activates a partial route, or deletes an unverified credential.
