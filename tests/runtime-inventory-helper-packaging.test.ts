...
buildTest("keeps the packaged Node helper free of Electron-only imports", async () => {
  ...
  const sources = await Promise.all(
    helperChunks.map((name) => readFile(join("out/main/chunks", name), "utf8")),
  );
  ...
  expect(builtHelper).not.toContain('require("original-fs")');
