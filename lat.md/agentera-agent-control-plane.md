...
[[tests/e2e/agentera-agent-control.e2e.ts]] also runs this path in a real Electron process: an injected first Runtime `profile create` interruption leaves a durable pending Installation and fresh-Profile reservation, while cold restart reuses the same Profile and Runtime Profile IDs. The injection fixture source is independent from the Runtime selected by Electron, so candidate acceptance can keep every device on the signed packaged Seed; the Electron process retains its host `HOME` while Runtime isolation remains bound to `HERMES_HOME`. The acceptance asserts the default Hermes root is byte-stable, a foreign Profile is untouched, no suffixed replacement is created, and the installation journal reaches `committed`.
