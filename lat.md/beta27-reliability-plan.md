...
### Dotenv plans match Hermes sanitizer

[[src/main/config.ts#planEnvValueWrite]] emits a terminal LF for every non-empty `.env` plan, matching Hermes startup sanitization so the Runtime cannot rewrite the planned bytes and trigger a false stale-plan rejection.
