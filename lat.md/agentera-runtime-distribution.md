...
The packaged contract has a dedicated bounded budget that includes first-install filesystem work, but its install, installed-file inspection, Profile binding, Gateway IPC, process, endpoint, capability, and shutdown boundaries remain independently timed. On Windows it emits a path-free JSONL timeline while the Gateway IPC is still pending, including verified listener-PID state and bounded process-tree cleanup phases. A failed Windows contract uploads that timeline, the Playwright log, and the isolated-run trace for fourteen days; Gateway and Electron tails are bounded and redact local paths, Bearer credentials, `API_SERVER_KEY`, tokens, and secrets. Successful candidates do not publish this diagnostic artifact.
