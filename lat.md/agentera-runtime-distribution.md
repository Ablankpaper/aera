...
[[src/main/agentera-runtime-distribution/extractor.ts#extractRuntimeArchive]] accepts only the signed platform format: TAR/Zstandard for macOS ARM64 and ZIP for Windows x64. Archive paths, normalized metadata, case-folded Windows duplicates, symlink targets, entry types, modes, sizes, and the decompression budget are checked against the signed inventory before a version can be published. Runtime ZIP members are pre-scanned without writing. In packaged Windows Electron, the external credential-free validator reads exactly the signed `archive_size` through a bounded sequence of native file-handle reads and parses it with yauzl's buffer reader; development and non-packaged paths retain the `original-fs` random-access reader. The packaged extraction helper then writes the validated archive into a fresh transaction with one yauzl reader and one output stream at a time. This deliberately avoids the native binding's parallel-write boundary observed in packaged Electron while keeping memory and process ownership bounded. The native ZIP binding remains available only for explicit diagnostic/native-mode controls and is not loaded by the production helper. Windows extraction temporarily disables Electron ASAR interception and always restores the prior setting. The packaged Windows app runs the final external staging-tree inventory verifier through the same Aera executable in `ELECTRON_RUN_AS_NODE` mode, with a generated helper outside `app.asar`, a one-use request file, and a credential-free environment; the helper uses native Node filesystem calls, `Dirent` entry kinds to avoid redundant per-entry stats, one bounded `readFile` for entries no larger than 256 KiB, positional file-handle reads for larger entries, and a bounded 32-file hash pool that dispatches the next file as each worker completes. This avoids Electron main-process filesystem interception and excessive per-entry handle round trips while preserving the same path, size, symlink, byte-budget, and SHA-256 checks. The fresh staging tree is walked exactly once without following links; every file is re-hashed through the bounded completion-driven pool and compared path-for-path with the manifest. POSIX hosts normalize and recheck non-Windows filesystem modes; Windows hosts skip that unrepresentable post-extraction check while retaining the signed archive-mode validation.
...
On Windows, the Desktop-managed Gateway also receives Runtime's existing `HERMES_GATEWAY_EXTERNAL_SUPERVISOR=1` marker. Electron is the external owner for this launch: it records identity, gates authenticated readiness, and performs bounded teardown. The marker prevents Runtime's service-conflict guard from running its unsupported Windows process-ancestry scan before initialization; it does not bypass the Desktop ownership, listener identity/image, readiness, or cleanup checks.
...
The diagnostic's traced chain uses the same packaged Python, managed site-packages working directory, optional Profile selector, and one diagnostic-only `-X importtime` flag. Its final phase is the exact `python -m hermes_cli.main gateway` dispatch with that tracing flag; `gateway-stacktrace` is appended only after its readiness timeout and enables bounded `faulthandler` output. For a cold-start comparison the workflow can instead run one direct phase with the unmodified `python -m hermes_cli.main gateway` argv, using either an isolated disposable home or the inherited packaged-runner HOME. A separate instrumented phase starts at process entry, enables periodic `faulthandler`, and records a bounded, path-free function-entry marker stream through the same dispatch. Its profiler stays on the launch thread, filters by function name before touching filenames, and never installs a process-wide thread callback; this keeps the probe from perturbing logging/argparse workers or changing the production readiness or ownership code.

The managed environment follows [[src/main/hermes.ts#buildGatewayEnv]] while replacing only the physical `HERMES_HOME` and API-server key/port with disposable values. Direct candidate-home mode preserves the inherited HOME/USERPROFILE/AppData roots that a packaged Electron child receives; fake-home mode is the isolated control. Process evidence retries only empty, timeout, query-error, or invalid-JSON Windows queries; a complete PID, creation identity, image, executable path, and command line remains required, and an identity mismatch is terminal. The diagnostic retains wrapper/listener evidence, every `gateway.pid` transition, authenticated capabilities results, import-time and faulthandler tails, startup marker tails, cleanup attempts, and residual PIDs. A live or unverified process keeps its sandbox, so `EBUSY` cannot erase the first-failure evidence.
...
#### Packaged install transaction lifecycle evidence

The packaged installer emits a path-free JSONL timeline for verification, transaction extraction, health, publication, activation, and transaction cleanup so a packaged delay can be assigned to one boundary.

[[src/main/runtime-install-lifecycle-diagnostics.ts#createPackagedSeedInstallDiagnostic]] enables this observer only when the acceptance diagnostic flag and an absolute output file are both present. It whitelists bounded Runtime version and disk-size fields, swallows sink failures, and never changes the installer result. [[tests/runtime-install-lifecycle-diagnostics.test.ts]] and [[tests/installer-runtime-install-diagnostics.test.ts]] cover the opt-in, redaction, failure-tolerance, and `runPackagedSeedInstall` wiring contracts.

With the explicit inventory diagnostic output enabled, final extracted verification emits path-free walk start/progress/complete and hash start/completion-pool progress/complete events with only counts and elapsed time. Hash progress counts completed files rather than waiting for a fixed batch to drain, so one slow filesystem read cannot hide work that has already completed in the other bounded worker slots. External observer failures are classified as timeout, output bound, nonzero exit, or spawn failure instead of a generic error. This evidence never changes hash concurrency, helper deadlines, inventory acceptance, or cleanup behavior.
...
Production-mode variants set `AGENTERA_RUNTIME_ARCHIVE_EXTRACTION_MODE=sequential`; explicit native variants remain as controls only.
...
start/complete/error timeline when explicitly enabled. The diagnostic-only
