...
[[src/main/agentera-runtime-distribution/extractor.ts#extractRuntimeArchive]] accepts only the signed platform format: TAR/Zstandard for macOS ARM64 and ZIP for Windows x64. Archive paths, normalized metadata, case-folded Windows duplicates, symlink targets, entry types, modes, sizes, and the decompression budget are checked against the signed inventory before a version can be published. Runtime ZIP members are pre-scanned without writing, then Electron's packaged native extractor writes the already verified archive into a fresh transaction with a bounded worker pool. Windows extraction and final inventory verification temporarily disable Electron ASAR interception for the external staging tree and always restore the prior setting. After the fresh staging tree is atomically placed at its transaction destination, it is walked exactly once without following links, re-hashed, and compared path-for-path with the manifest. POSIX hosts normalize and recheck non-Windows filesystem modes; Windows hosts skip that unrepresentable post-extraction check while retaining the signed archive-mode validation.
