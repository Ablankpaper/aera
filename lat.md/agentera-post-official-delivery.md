...
The Windows disposable install/start/update/rollback smoke binds every wait and cleanup operation to the exact root PID returned by `Start-Process`, then performs a bounded same-executable sweep so Electron children reparented during shutdown cannot leave an install lock. A failure reports only a stable smoke stage and code, while the script remains disposable and never selects arbitrary same-path processes as the update root.
