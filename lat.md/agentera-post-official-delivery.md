...
The Beta.20 internal-Beta path requires a Developer-ID-signed macOS candidate but records a one-version notarization deferral while Windows remains explicitly unsigned for trusted-company testing.

`.github/workflows/internal-beta.yml` requires one exact successful-CI Desktop SHA on `main`, prepares and verifies the locked Runtime Seed without checking out Runtime source, and bakes one reviewed HTTPS IP Origin plus its issuer-scoped Ed25519 public key. For Beta.20 the macOS job requires protected Developer ID credentials, forces real code signing, verifies strict `codesign`, native architecture and Runtime Seed identity, and records that notarization, stapling and Gatekeeper acceptance are deferred rather than fabricating those results. The Windows job disables identity discovery and remains unsigned. Tags and Releases stay disabled, and a separate pinned Ed25519 key authenticates the private online-update channel. Production candidate notarization remains strict and unchanged.

`scripts/internal-beta/manifest.mjs` binds version `0.7.4-internal-beta.20`, Runtime Seed candidate `runtime-v0.18.2-agentera.1-rc.4` at `dcb0f0bc6a0e2d18c55beedc6517dbc41d8b01e0`, both workflow run identities, the Origin and public trust root, Runtime lock and Darwin/Windows manifests, four fixed package names and hashes, SPDX SBOM, SLSA v1 provenance, `macos-evidence.json`, and `macos_developer_id_signed_notarization_deferred_windows_unsigned`. The workflow additionally signs a strict two-platform Desktop update manifest, publishes through a forced-command-only principal, atomically switches current metadata, and verifies the live path. Runtime stable upgrades remain a separate user-confirmed channel, so the packaged `.1` Seed is not manually replaced by the published `.3` Runtime. Beta.6 through Beta.8 require a manual bridge install because they predate the corrected macOS ASAR extraction boundary.
...
arbitrary notes, or logs. Beta.20 and later macOS candidates must pass
