...
The CI workflow also exposes manual `windows-process-tree-diagnostic`, `windows-managed-gateway-diagnostic`, and `windows-runtime-health-diagnostic` modes for release-isolated hosted-Windows process, packaged-Gateway, and Runtime-health checks. Their distinct job names and missing macOS/Ubuntu matrix mean `scripts/verify-ci-checkpoint.mjs` cannot accept them as a release checkpoint; push, pull-request, and full manual CI retain the complete matrix. Candidate validators allow only these exact diagnostic jobs to be skipped beside a full run and still require the named Ubuntu, macOS, and Windows `check` jobs to execute successfully, so diagnostic-only or incomplete runs remain ineligible for packaging.
