...
The CI workflow also exposes manual `windows-process-tree-diagnostic`, `windows-managed-gateway-diagnostic`, and `windows-runtime-health-diagnostic` modes for release-isolated hosted-Windows process, packaged-Gateway, and Runtime-health checks. Their distinct job names and missing macOS/Ubuntu matrix mean `scripts/verify-ci-checkpoint.mjs` cannot accept them as a release checkpoint; push, pull-request, and full manual CI retain the complete matrix. Candidate validators allow only these exact diagnostic jobs plus the conditionally skipped `windows-packaged-acceptance` job beside a full run, and still require the named Ubuntu, macOS, and Windows `check` jobs to execute successfully, so diagnostic-only or incomplete runs remain ineligible for packaging.
...
Beta.38 keeps signed and notarized macOS packages while publishing Windows setup and portable bytes as explicitly unsigned controlled internal-Beta artifacts. Production candidates remain Authenticode fail-closed.

`.github/workflows/internal-beta.yml` requires one exact successful-CI Desktop SHA on `main`, prepares and verifies the candidate Runtime Seed `0.20.0-agentera.5` from Runtime source `88e569614a07aee3036ab36c034798e5a4a2b884` without checking out Runtime source, and bakes one reviewed HTTPS IP Origin plus its issuer-scoped Ed25519 public key. The macOS job requires protected Developer ID and App Store Connect credentials, accepted notarization, stapling, Gatekeeper, architecture, Runtime Seed, and final-container verification. The Beta.38 Windows job disables signing discovery, verifies setup and portable signatures are `NotSigned`, and still requires x64 identity, Runtime Seed verification, exact bytes, startup, update, and rollback evidence. Its package evidence accepts only PE machine `0x014C` or `0x8664` for the NSIS wrapper, while the packaged application, Runtime Seed, and native modules remain strictly x64. Windows identity reads the full prerelease from `FileVersion`, since `ProductVersion` is normalized to four numeric components. Tags and Releases stay disabled, and a separate pinned Ed25519 key authenticates the private online-update channel.

The Windows disposable install/start/update/rollback smoke binds each launch to the exact root PID returned by `Start-Process`. Setup and portable cleanup repeatedly terminate only their exact executable, while helper-driven swaps stop only the old root so the smoke cannot kill the newly swapped same-path candidate. The production helper also waits for both the old tree and any same-executable process to disappear before moving the install directory. A failure reports only a stable path-free substage and code; physical update acceptance remains separate.

`scripts/internal-beta/manifest.mjs` uses a schema-3 fail-closed contract for the current Beta.38 candidate line. It binds both workflow run identities, Origin and public trust root, Runtime lock and Darwin/Windows manifests, five fixed package names and hashes, SPDX SBOM, SLSA provenance, macOS notarization evidence, Windows `unsigned_internal_beta` evidence, two packaged startup proofs, and one native inventory extracted from each final distributable. Every inventory must match the exact container SHA-256/SHA-512, source SHA, version, platform, architecture, Electron ABI, payload, and native-module hashes. The candidate workflow additionally signs the two-platform Desktop update manifest and uploads one immutable artifact without publisher credentials; promotion verifies those exact bytes and never rebuilds or resigns.
