...
Dashboard invocation unit tests isolate process-tree termination as well as the spawned child. A fake PID must never reach the host `process.kill`, where a coincidentally live POSIX process group could terminate the Vitest worker instead of the mock Runtime.
...
A brand-new chat does **not** load `model.options` before its first prompt. That inventory can perform remote catalog/pricing/capability reads and probe the current custom provider, so an empty cache on a newly installed Mac or Windows machine can exceed the request timeout even though the configured model itself is usable. The desktop instead sends the selected `model`, `provider`, and non-secret `base_url` on `session.create`; Runtime builds that lazy session directly with the per-session override, preserving the configured API protocol when a bare custom endpoint matches the profile default. The transport records that exact runtime selection and proceeds to `prompt.submit`, which creates the durable DB row before the asynchronous model turn. This keeps cold-cache model discovery out of the send critical path and lets the new conversation appear in the sidebar as soon as the first prompt is accepted.
...
Each `message.complete` is accepted once per renderer turn, and verified event provenance decides whether final text replaces or merges with streamed text.

A valid sequenced stream remains governed by `stream_id`/sequence/digest integrity and its verified full text replaces the bubble exactly. For a legacy or unsequenced turn with no tool event, a non-empty completion is also the authoritative whole answer: it replaces the accumulated deltas instead of joining two complete-looking answers into one bubble. The renderer keys this lifecycle to the local `ActiveTurn.turnId`; after accepting one completion it ignores another completion replay for that turn, while the next prompt's new turn id resets the guard even when the later answer text is identical.

Tool-bearing legacy turns retain the established reconciliation because a last-turn-only final would otherwise clobber useful prose streamed before a tool call (#746). [[src/renderer/src/screens/Chat/dashboardEventAdapter.ts#completeAssistantWithFinalText]] rewrites the last assistant bubble through [[src/renderer/src/screens/Chat/dashboardEventAdapter.ts#mergeStreamedWithFinal]], which compares whitespace-insensitively and: uses the final text when it already contains the streamed text; keeps the streamed text when it contains the final; stitches a re-streamed boundary by dropping the duplicated word-aligned seam (rejecting coincidental mid-word overlaps); replaces a garbled re-stream with the final text when the two converge on a substantial common suffix; and otherwise concatenates the pre-tool and final segments with a blank-line separator. `thinking.delta` remains excluded, and reasoning stays in its own row. On the remote/SSH path deltas are not rendered (`renderAssistantDeltas: false`), so the bubble starts empty and the final text is used verbatim.
