...
The current release integration identity is `0.7.4-internal-beta.34`; Beta.31, Beta.32, and Beta.33 remain explicit source versions for supported update and rollback scenarios, while Beta.29 remains a manual-DMG bridge only.
...
- Beta.34 Internal Beta Windows setup and portable artifacts must verify as explicitly unsigned; their exact bytes, the x64 app ZIP, locked Runtime Seed, packaged startup proof, final native inventories, unsigned-status evidence, and signed update metadata are all bound to the canonical manifest.
