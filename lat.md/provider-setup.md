...
When [[src/renderer/src/components/ConfigHealthBanner.tsx#ConfigHealthBanner]] sees that the credential is missing in Local mode, it asks the Main Process to generate it, re-runs configuration health, and removes the notice after success. The Renderer receives only whether generation occurred, never the generated value.

Automatic generation is refused in Remote and SSH modes because those connections own their authentication separately. A profile using `secrets.provider: command` is refreshed but never overwritten; if its provider still cannot resolve the internal credential, the UI offers a human-readable retry and diagnostic details.
...
The General models path is intentionally beginner-facing. **Add model** opens one preset/custom dialog: presets fill Provider, Base URL, credential slot, and API protocol; custom mode exposes name, endpoint, optional context length, and API mode. The **Fetch** control sits beside the default-model field, calls `discoverProviderModels`, and keeps manual model entry available when a service has no catalog endpoint.

Saving the dialog stores the key in the provider-specific env slot, persists every returned model in `models.json`, writes the selected model through `setModelConfig`, and immediately activates it. Petoi is a curated preset at `https://api.petoi.cn/v1` and always stores its credential in `PETOI_API_KEY`.

Each configured service renders as a detailed card rather than a one-line provider row. The card exposes only non-secret identity and model data: service name, preset/custom and current-default badges, Provider route, Base URL, model count, a real default-model selector, and model tags. Its actions call real operations to set the global default, refresh and persist the discovered catalog, or reopen the existing configuration dialog. API keys never render on the saved card.
