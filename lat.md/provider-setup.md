...
Each configured service renders as a detailed card rather than a one-line provider row. The card exposes only non-secret identity and model data: service name, preset/custom and current-default badges, Provider route, Base URL, model count, a real default-model selector, and model tags. Setting an already-saved model as default calls the managed `setModelConfig` activation path and does not replay a stale catalog upsert; refresh/edit remain the operations that persist service inventory. API keys never render on the saved card.
